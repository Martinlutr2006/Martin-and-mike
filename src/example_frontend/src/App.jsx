...
function App() {
  ...
  const fetchStudentCourses = async (studentId) => {
    try {
      const studentCoursesList = await example_backend.getStudentCourses(studentId);
  ...
  const handleAssignCourseToStudent = async () => {
    console.log("Assigning course:", selectedCourseId, "to student:", selectedStudentId);

    try {
      await example_backend.assignCourseToStudent(selectedStudentId, selectedCourseId);
        ...
        <>
          ...
          <label>
            ...
            <select value={selectedStudentId} onChange={(e) => setSelectedStudentId(e.target.value)}>
              ...
              {students.map((student, index) => (
                <option key={index} value={student.firstName + ' ' + student.lastName}>{student.firstName} {student.lastName}</option>
              ))}
            </select>
          </label>
          <label>
          ...
          {selectedStudentId && (
            <>
              ...
              <button onClick={() => fetchStudentCourses(selectedStudentId)}>Fetch Courses for Student</button>
              <ul>
